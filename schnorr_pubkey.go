...
func (key *SchnorrPublicKey) Serialize() (*SerializedSchnorrPublicKey, error) {
	...
	_ = C.secp256k1_xonly_pubkey_serialize(C.secp256k1_context_no_precomp, cPtr, &key.pubkey)
	/*if ret != 1 {
...
func (key *SchnorrPublicKey) addInternal(tweak [32]byte) (bool, error) {
	...
	_ = C.secp256k1_xonly_pubkey_tweak_add(context, &fullKey, &key.pubkey, cPtrTweak)

	var cParity C.int
	_ = C.secp256k1_xonly_pubkey_from_pubkey(context, &key.pubkey, &cParity, &fullKey)
